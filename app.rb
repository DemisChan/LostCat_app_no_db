...
class WebApplicationServer < Sinatra::Base
  ...
  get '/lostcats' do
    erb :lostcats_index, locals: {
      cats: cat_list.report
    }
  end
  ...
  post "/lostcats" do
    cat = Cat.new(params[:name], params[:breed], params[:tel], params[:password])
    cat_list.add(cat)
    redirect "/lostcats"
  end

  delete "/lostcats/:index" do
    cat_list.remove(params[:index].to_i)
    redirect "/lostcats"
  end

  get "/lostcats/:index/edit" do
    erb :lostcats_edit, locals: {
      index: params[:index].to_i, 
      cats: cat_list.get(params[:index].to_i)
    }
  end

  patch "/lostcats/:index" do
    cat = Cat.new(params[:name], params[:breed], params[:tel], params[:password])
    cat_list.update(params[:index].to_i, cat)
    redirect "/lostcats"
  end

  # This is an example of setting up a new instance using the global data store.
  def cat_list
    $global[:lostcats] ||= CatsList.new
  end
